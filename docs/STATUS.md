...
| **8.5** | Load-bearing arithmetic correctness via Rust runner | `cargo run --release --bin run_gate1 -- --primitive <p> --count <n>` | ✅ 4/5 short primitives at 1000-vector depth: `byte_add` 1000/1000 in 19.6s, `byte_sub` 1000/1000 in 254s, `transfer_finalize` 1000/1000 in 576s, `transfer_check` 1000/1000 in 3114s. `mpt_emit_record` re-running solo (initial parallel run was CPU-starved; 100/100 partial passed). `freeze_setup` / `freeze_apply` deferred (10k vectors at current Rust perf is days; needs SIMD attention). Logs in `docs/gate85_logs/`. | _(in flight)_ |
