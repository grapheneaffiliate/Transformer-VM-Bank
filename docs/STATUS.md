...
| **8** | Pure-Rust runner — canonical engine | `cargo test -p psl-rust-runner --test parity --release -- --ignored` + `tools/run_canonical_gate1.sh` | ⚠️ partial — Rust runner is the canonical reference for trace-hash production (`docs/ARCHITECTURE.md § 0.3`). Validated against gate-1 vectors at 10k each on the 5 short primitives: `byte_add` 10000/10000 (37.9s), `byte_sub` 10000/10000 (470s), `transfer_finalize` 10000/10000 (1184s), `transfer_check` 10000/10000 (3114s), `mpt_emit_record` 10000/10000 (4961s) — **50000/50000, 0 failures**. `freeze_setup` and `freeze_apply` validated only via the original gate 1 sweep (C++ engine, 10000/10000 each). Re-running them under the Rust runner is bound by O(t²) attention growth (17k tokens for setup, 7.7k for apply) and remains pending sparse-attention work (see `rust_runner/src/sparse.rs` and `docs/FINDINGS.md` § Gate 8 follow-up). Cross-engine algorithmic match holds against `transformer.cpp`'s Linux build (`#else` in `matvec`). | _(this commit)_ |
