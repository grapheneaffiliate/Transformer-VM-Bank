...
| **8** | Pure-Rust runner — canonical engine | `cargo test -p psl-rust-runner --test parity --release -- --ignored` + `tools/run_canonical_gate1.sh` | ✅ Pure-Rust runner is the canonical reference for trace-hash production (`docs/ARCHITECTURE.md § 0.3`). Re-validated against gate-1 vectors at 10k each on the 5 short primitives (50000/50000, 0 failures) and the chained `freeze_setup → freeze_apply` pipeline. Cross-engine algorithmic match holds against `transformer.cpp`'s Linux build (`#else` in `matvec`). PyTorch+MKL parity at scale is architecturally out-of-scope and not a correctness criterion. | _(this commit)_ |
