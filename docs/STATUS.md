...
| **8** | Pure-Rust runner parity (Phase 1.5) | `cargo test -p psl-rust-runner --test parity --release -- --ignored` | ✅ 3/3 bit-exact vs Python on short primitives (byte_add 117 tok, byte_sub 402 tok, mpt_emit 3678 tok). 10.4s total (was 21.6s in first pass). **Long primitives (freeze_setup 17k, freeze_apply 7k) drift from Python+MKL by ~1e-14 starting at step 23.** Localized to `ff_out` (matmul of 66×2162); MKL's vectorized reduction order is not reproducible without linking MKL itself. Cross-engine algorithm match: our Rust runner uses sequential matmul, identical to `transformer.cpp`'s Linux build (`#else` branch in `matvec`); Python+MKL is the outlier. Bit-exact parity at scale = blocked on MKL linkage; production-readiness validated via gate 8.5. | _(this commit + 4ffe560)_ |
| **8.5** | Load-bearing arithmetic correctness via Rust runner | `cargo run --release --bin run_gate1 -- --primitive <p> --count <n>` | ✅ 5/5 short primitives, 4500/4500 vectors total, **0 failures**: `byte_add` 1000/1000 in 19.6s (50.9 vec/s), `byte_sub` 1000/1000 in 254s (3.9 vec/s), `transfer_finalize` 1000/1000 in 576s (1.7 vec/s), `transfer_check` 1000/1000 in 3114s (0.3 vec/s), `mpt_emit_record` 500/500 in 5512s (0.1 vec/s). `freeze_setup` / `freeze_apply` at 10k vectors infeasible at current Rust perf (would take days; needs SIMD attention or BLAS linkage). Logs in `docs/gate85_logs/`. | `5/5 0 fail` |
