...
| **17** | External security audit (Trail of Bits / Zellic / OtterSec) | hand-off package: `docs/SECURITY_REVIEW.md` + `tools/sbom.sh` + `deny.toml` | 🟡 audit-ready package shipped — formal threat model per layer, 10 named invariants with test pointers, attack-surface inventory by layer, recommended audit scope (read-through + fuzz + property tests + cross-platform determinism + crypto hygiene), pinned dependencies via `Cargo.lock` + cargo-deny config, SBOM script. **Awaits external auditor engagement.** Findings will land in `docs/AUDIT_FINDINGS.md` and the report itself in `docs/audits/<date>_<vendor>.pdf`. | _(this commit)_ |
| **18** | Production-readiness review (monitoring, alerting, runbooks, DR) | `docs/OPERATIONAL_READINESS.md` | 🟡 ops package shipped — service inventory + SLOs, Prometheus metric schemas per long-running component, alerting thresholds for sequencer halt / state-root mismatch / mempool overflow / signing-key compromise, runbooks for the four named incidents, blue-green rollback procedure, hourly snapshot + 90-day cold retention DR plan, quarterly restore drill, structured logging schema, pre-deployment checklist. **Pre-deployment sign-off requires the gate-17 audit report.** | _(this commit)_ |
