...
| **10** | Ternary execution engine — Phase 2 Layer 1 | `cargo test -p psl-ternary-vm --release` | 🟡 in progress — `byte_add_with_carry` shipped: 131072/131072 exhaustive (every (a,b,c_in) combo), 1.25s wall on a single thread (~105k vec/s). Crate `ternary_vm/` includes `SparseTernaryLayer`, `TernaryNetwork`, `argmax`, BLAKE3-hashed packed weight format, `trace_hash_ternary` (`docs/ARCHITECTURE.md § 0.8`), checked-arithmetic forward kernel (no panics in production paths). Remaining 6 primitives — byte_sub, freeze_apply, transfer_finalize, transfer_check, mpt_emit_record, freeze_setup — follow the same thermometer construction pattern; landing primitive-by-primitive. | _(this commit)_ |
