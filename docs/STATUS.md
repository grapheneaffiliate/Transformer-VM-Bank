...
| **19** | Post-quantum cryptographic agility (Phase G phase 1) | `cargo test -p psl-crypto-agility --release` | 🟡 agility infrastructure shipped — 23 unit tests + 6 proptests pass. New crate `crypto_agility/` defines `SignatureScheme` / `KemScheme` / `HashScheme` enums with reserved discriminants for `HybridEd25519MlDsa65` / `HybridX25519MlKem768` / `Blake3_512`; `Signer` / `Verifier` / `Kem` / `HashScheme_` traits; `VerifierPolicy` (ed25519-only / migration-window / hybrid-only); ed25519 implementation; BLAKE3-256 + BLAKE3-512 implementations; LEB128 varint scheme prefix codec; explicit `UnknownScheme` rejection (no silent fallback). Three ADRs ratify the strategy: **[ADR-0006](decisions/0006-post-quantum-cryptography-strategy.md)** PQ strategy (hybrid ed25519 + ML-DSA-65 / X25519 + ML-KEM-768; FN-DSA excluded permanently for fp-incompat), **[ADR-0007](decisions/0007-cryptographic-agility-architecture.md)** agility traits + wire format + hash-of-pubkey state-tree, **[ADR-0008](decisions/0008-blake3-512-for-long-lived-commitments.md)** BLAKE3-512 for `weights_hash` + long-lived contract hashes only. **Pending (Phase G phases 2-6):** pqcrypto-mldsa + pqcrypto-mlkem integration, hybrid signature/KEM impls, refactor of existing call sites through the trait, agent-layer migration, ed25519-deprecation tooling, external cryptographer review. | _(this commit)_ |
