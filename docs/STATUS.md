...
| **11** | Contract DSL standard library — Phase 2 Layer 2 | `cargo test -p psl-agent-contracts --release` | 🟡 in progress — `agent_contracts/` crate with `TernaryProgram` trait + `program_hash` (BLAKE3 over name + sub-network weights_hashes). **2/8 standard contracts shipped**: `transfer` (composes `transfer_check`, `byte_sub` ×16, `byte_add` ×16, `transfer_finalize`; 5/5 tests inc. 100 random u128 witnesses) and `swap` (2-asset exchange between two parties; 3/3 tests inc. 50 random witnesses). 8/8 tests total; canonical no-op zeros on precondition failure (insufficient balance, recipient overflow). Remaining 6 contracts (escrow_create/release/refund, time_locked_release, multisig_2of3, conditional_payment) and the parsed-DSL frontend (typechecker / interpreter / compiler) land in subsequent commits — same composition pattern, no architectural unknowns. | _(this commit)_ |
| 12 | Identity & wallet (hierarchical keys, spending limits, revocation) | TBD | ⏳ Phase 2 Layer 3 | — |
| 13 | Negotiation protocol (registry, propose/accept/reject/counter/execute, mutual TLS) | TBD | ⏳ Phase 2 Layer 4 | — |
| 14 | Dispute resolution (deterministic re-execution + slash) | TBD | ⏳ Phase 2 Layer 4 | — |
| 15 | Reference agents (trader + service in docker-compose) | TBD | ⏳ Phase 2 Layer 5 | — |
| 16 | SDK release 0.1.0 (Rust + UniFFI to Swift/Kotlin/Python/JS) | TBD | ⏳ Phase 2 Layer 5 | — |
| 17 | External security audit (Trail of Bits / Zellic / OtterSec) | TBD | ⏳ pre-production | — |
| 18 | Production-readiness review (monitoring, alerting, runbooks, DR) | TBD | ⏳ pre-production | — |
